...
<title>Dev Tuấn Dương — Profile v2</title>
...
:root{
  --bg:#060014;
  --fg:#ffffff;
  --cyan:#00bfff; /* xanh dương neon */
  --pink:#ff69b4; /* hồng neon */
  --glow:0 0 25px rgba(0,191,255,.45),0 0 60px rgba(255,105,180,.25);
  --card-bg:rgba(10,10,30,.5);
}
html,body{
...
.panel:hover{box-shadow:0 0 50px var(--pink),0 0 100px var(--cyan); transform:scale(1.02)}

.head{display:flex; align-items:center; gap:16px}
.avatar{position:relative; width:96px; height:96px; border-radius:50%; overflow:hidden; border:2px solid var(--pink); box-shadow:var(--glow); cursor:pointer; transition:.3s}
.avatar::after{content:""; position:absolute; inset:-6px; border-radius:50%;
  background:conic-gradient(from 0deg, rgba(0,191,255,.3), rgba(255,105,180,.3), rgba(0,191,255,.3));
  filter:blur(14px); z-index:-1; animation:halo 6s linear infinite}
@keyframes halo{to{transform:rotate(360deg)}}
.avatar:hover{box-shadow:0 0 60px var(--pink),0 0 100px var(--cyan); transform:scale(1.08)}
.avatar img{width:100%; height:100%; object-fit:cover}
.online{position:absolute; right:6px; bottom:6px; width:12px; height:12px; border-radius:50%; background:linear-gradient(135deg,var(--cyan),var(--pink)); box-shadow:0 0 10px var(--pink)}

.title h1{margin:0; font-size:1.6rem; letter-spacing:.3px; color:var(--pink); text-shadow:0 0 14px currentColor; display:flex; align-items:center; gap:8px}
...
.badge{font-size:.9rem; padding:6px 10px; border-radius:10px; background:rgba(0,0,0,.25); border:1px solid rgba(255,105,180,.35); color:var(--fg)}
.badge a{color:inherit; text-decoration:none}
.bio{margin:12px 0 16px; color:#ffdfff}

.actions{display:grid; grid-template-columns:repeat(2,1fr); gap:10px}
.btn{padding:12px 10px; border-radius:12px; border:1.6px solid var(--pink); background:transparent; color:var(--pink); font-weight:700; cursor:pointer; transition:.25s transform,color,box-shadow; text-align:center}
.btn:hover{transform:translateY(-2px); background:radial-gradient(120% 120% at 50% 10%, rgba(255,105,180,.22), rgba(0,191,255,.22)); box-shadow:var(--glow); color:#001217}

.section{margin:4px 0 12px; color:var(--pink); font-weight:800; text-shadow:0 0 10px var(--pink); position:relative}
.section::after{content:""; position:absolute; left:0; bottom:-6px; width:72px; height:3px; background:linear-gradient(90deg,var(--pink),var(--cyan)); border-radius:2px}
.grid{display:grid; grid-template-columns:repeat(3,1fr); gap:12px}
@media (max-width:900px){ .grid{grid-template-columns:repeat(2,1fr)} }
@media (max-width:520px){ .grid{grid-template-columns:1fr} }
.tile{display:flex; justify-content:center; align-items:center; gap:8px; padding:14px 12px; border-radius:14px; text-decoration:none; color:var(--fg); background:rgba(0,0,0,.35); border:1px solid rgba(255,105,180,.25); transition:.25s all}
.tile:hover{background:radial-gradient(circle at center, rgba(255,105,180,.25), rgba(0,191,255,.2)); box-shadow:var(--glow); transform:scale(1.05)}
.footer{text-align:center; margin-top:18px; color:#bfefff}

.toast{position:fixed; bottom:20px; left:50%; transform:translateX(-50%); background:linear-gradient(135deg,var(--pink),var(--cyan)); color:#001217; padding:10px 16px; border-radius:12px; font-weight:800; display:none; z-index:5; box-shadow:var(--glow)}
...
<div class="wrap">
  <div class="shell">
    <section class="panel" aria-label="Hồ sơ">
      <div class="head">
        <div class="avatar" id="avatar" title="Nhấn để thả tim">
          <!-- Avatar giữ nguyên -->
          <img src="https://i.imgur.com/NAYm1TG.jpeg" alt="Avatar" loading="lazy" />
          ...
          <h1>Dev Tuấn Dương
            <span class="badge-verify" title="Verified">
              <svg width="22" height="22" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                ...
                <defs><linearGradient id="g" x1="0" y1="0" x2="24" y2="24" gradientUnits="userSpaceOnUse"><stop stop-color="#ff69b4"/><stop offset="1" stop-color="#00bfff"/></linearGradient></defs>
              </svg>
            </span>
          </h1>
      ...
      <div class="badges">
        <span class="badge">📍 Tuyên Quang, Việt Nam</span>
        <span class="badge">☎ Ẩn danh</span>
        <span class="badge">✉ tuanduongdeptrai@gmail.com</span>
      </div>
      <p class="bio">💻 Developer | Tool | WAR | DDOS 💕</p>
      <div class="actions">
        <a class="btn" href="https://www.facebook.com/haomundev" target="_blank" rel="noopener">LIÊN HỆ</a>
    ...
    <section class="panel" aria-label="Liên kết & mạng xã hội">
      ...
      <div class="grid">
        <a class="tile" href="https://www.tiktok.com/@_trduong19_?_r=1&_t=ZS-91F09RvSMjw" target="_blank" rel="noopener">🎵 <strong>TikTok</strong></a>
        <a class="tile" href="https://www.youtube.com/@noicamxucbocdau" target="_blank" rel="noopener">▶ <strong>YouTube</strong></a>
        <a class="tile" href="https://www.facebook.com/tuanduongnek" target="_blank" rel="noopener">📘 <strong>Facebook</strong></a>
        <a class="tile" href="https://zalo.me/0559705922" target="_blank" rel="noopener">💬 <strong>Zalo</strong></a>
      </div>
      <div class="footer">© 2025 • Dev Tuấn Dương | HAC LONG Team</div>
    </section>
  </div>
</div>
...
function draw(){ctx.clearRect(0,0,W,H);
  const grd=ctx.createRadialGradient(W*.7,H*.3,0,W*.7,H*.3,Math.max(W,H)*.8); grd.addColorStop(0,'rgba(255,105,180,.08)'); grd.addColorStop(1,'transparent'); ctx.fillStyle=grd; ctx.fillRect(0,0,W,H);
  ...
  ctx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue('--pink'); ctx.lineWidth=1.6;
